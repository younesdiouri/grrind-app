...
never merges. While it is working, the primary thread must not edit the same scope in parallel.
The primary thread waits for the handoff, reviews the resulting diff and validation evidence
against the ticket and project invariants, then sends precise correction instructions back to the
